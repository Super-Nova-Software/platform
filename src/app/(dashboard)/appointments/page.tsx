...
const Page = async () => {
  ...
  const bookingsExistToday = domainBookings.bookings.filter(
    (booking: { date: { getDate: () => number } }) => booking.date.getDate() === today.getDate()
  )
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 flex-1 h-0 gap-5">
        ...
        <div className="col-span-1">
          ...
          {bookingsExistToday.length ? (
            bookingsExistToday.map((booking: { id: React.Key | null | undefined; slot: string | number | bigint | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | Promise<React.AwaitedReactNode> | null | undefined; createdAt: { getHours: () => number; getMinutes: () => number }; Customer?: { Domain?: { name?: string } }; email: string }) => (
              <Card
                key={booking.id}
